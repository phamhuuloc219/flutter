...
class PageChiTietSPFirebase extends StatelessWidget {
  PageChiTietSPFirebase({required this.sp,super.key});
  Fruit sp;
  @override
  Widget build(BuildContext context) {
    double w = MediaQuery.of(context).size.width*0.9;
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        child: Icon(Icons.add_shopping_cart,color: Colors.red,),
        onPressed: () {
          FirebaseController.instance.themVaoGH(sp,
            updateWidgetIds: ["gioHang"],
            function:(slSP){
              ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
...
class GioHang_Widget2 extends StatelessWidget {
  const GioHang_Widget2({
    super.key,
  });
